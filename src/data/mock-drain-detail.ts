...
/** 배수구 코드(manageNo, 예: AA-013)로 상세에서 drain id 찾기 */
export function getDrainIdByManageNo(code: string): string | null {
  const normalized = code.trim().toUpperCase();
  const entry = Object.values(MOCK_DRAIN_DETAIL).find(
    (d) => d.manageNo.toUpperCase() === normalized
  );
